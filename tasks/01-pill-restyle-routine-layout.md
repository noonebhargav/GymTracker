...
- **Picker → Explore-style cards** (`routine.tsx`): replaced the single-column list with a 2-col card grid mirroring the Explore body-part cards (`rounded-[14px] border p-4`, 50% width, `bg-card`). Same 3-state treatment (selected = `bg-primary/10` + accent + Check; covered = dimmed; unassigned = bright). Subtitle shows the assigned days (`Mon · Wed`) instead of an exercise count, or `Not assigned` when empty — unifying the Routine and Explore visual language.
